...
MARCDUINO_ACTION(MDFrontHoloOn, @6T1, ({
    // Front Holo white
    CommandEvent::process(F("HPF0029"));
}))
...
MARCDUINO_ACTION(MDTopHoloOn, @7T1, ({
    // Top Holo white
    CommandEvent::process(F("HPT0029"));
}))
...
MARCDUINO_ACTION(MDRearHoloOn, @8T1, ({
    // Rear Holo white
    CommandEvent::process(F("HPR0029"));
}))
...
MARCDUINO_ACTION(MDAllHolosRed, @9T1, ({
    // ALl Holos red
    CommandEvent::process(F("HPF0041"));
    CommandEvent::process(F("HPT0041"));
    CommandEvent::process(F("HPR0041"));
}))
