...
uint8_t DEFAULT_SERVO_DIRECTION[] = {
	1, // Data Panel Door
	0, // Upper Utility Arm
	0, // Lower Utility Arm
	1, // Left Bread Pan Door
	1, // Left Arm Raise/Lower
	0, // Left Arm Tool
	0, // Right Bread Pan Door
	0, // Right Arm Raise/Lower
	1, // Right Arm Tool
	0, // Charge Bay Door
	0 // Not Used
};
...
int main(void) {
	...
	for (int i=0; i<SERVO_NUM; i++)
	{
		...
		{
			temp = DEFAULT_SERVO_DIRECTION[i];
		}
...
#if _FEEDBACK_MSG_ ==1
const char strSeqUtilityArmsOpenClose[] PROGMEM="(Utility Arms Open Close) \r\n";
const char strSeqTestAll[] PROGMEM="(Test All Servos) \r\n";
const char strSeqWiggle[] PROGMEM="(Wiggle All Doors) \r\n";
const char strSeqUseGripperArm[] PROGMEM="(Use Gripper Arm) \r\n";
const char strSeqPingPongBreadPanDoors[] PROGMEM="(Ping Pong Bread Pan Doors) \r\n";
const char strSeqKnightRider[] PROGMEM="(Knight Rider) \r\n";
#endif
...
void sequence_command(uint8_t value)
{
	...
	{
		///////////////////////////////////////////
		//	sequences of body panels
		//
		//	:SE30 Utility arm open/close
		//	:SE31 Test all doors/tools
		//	:SE32 Wiggle body doors
		//	:SE33 Use gripper arm
		//	:SE34 Use interface arm
		//	:SE35 Ping-pong bread pan doors
		//	:SE36 Knight rider
		//////////////////////////////////////////

		case 30: // UTILITY ARMS
			seq_stopsequence(); 				// abort any previous sequence immediately
			seq_loadsequence(utility_arms_open_close, SEQ_SIZE(utility_arms_open_close));
			seq_loadspeed(panel_medium_speed);
			HPFlicker(4);
			seq_startsequence();
#if _FEEDBACK_MSG_ == 1
			serial_puts_p(strSeqUtilityArmsOpenClose);
#endif
			break;
		case 31: // TEST ALL
			seq_stopsequence(); 				// abort any previous sequence immediately
			seq_loadsequence(all_test, SEQ_SIZE(all_test));
			seq_loadspeed(panel_medium_speed);
			seq_startsequence();
#if _FEEDBACK_MSG_ == 1
			serial_puts_p(strSeqTestAll);
#endif
			break;
		case 32: // WIGGLE
			seq_stopsequence(); 				// abort any previous sequence immediately
			seq_loadsequence(panel_wiggle, SEQ_SIZE(panel_wiggle));
			seq_loadspeed(panel_fast_speed);
			seq_startsequence();
#if _FEEDBACK_MSG_ == 1
			serial_puts_p(strSeqWiggle);
#endif
			break;
		case 33: // GRIPPER ARM
			seq_stopsequence(); 				// abort any previous sequence immediately
			...
			seq_startsequence();
#if _FEEDBACK_MSG_ == 1
			serial_puts_p(strSeqUseGripperArm);
#endif
			break;
		case 34: // INTERFACE ARM
			seq_stopsequence(); 				// abort any previous sequence immediately
			seq_loadsequence(panel_use_interface_tool, SEQ_SIZE(panel_use_interface_tool));
			seq_loadspeed(panel_fast_speed);
			seq_startsequence();
#if _FEEDBACK_MSG_ == 1
			serial_puts_p(strSeqUseInterfaceArm);
#endif
			break;
		case 35: // PING PONG BREADPAN DOORS
			seq_stopsequence(); 				// abort any previous sequence immediately
			seq_loadsequence(panel_pingpong_breadpan_doors, SEQ_SIZE(panel_pingpong_breadpan_doors));
			seq_loadspeed(panel_fast_speed);
			seq_startsequence();
#if _FEEDBACK_MSG_ == 1
			serial_puts_p(strSeqPingPongBreadPanDoors);
#endif
			break;
		case 36: // KNIGHT RIDER
			seq_stopsequence(); 				// abort any previous sequence immediately
			seq_loadsequence(panel_knight_rider, SEQ_SIZE(panel_knight_rider));
			seq_loadspeed(panel_fast_speed);
			seq_startsequence();
#if _FEEDBACK_MSG_ == 1
			serial_puts_p(strSeqKnightRider);
#endif
			break;
